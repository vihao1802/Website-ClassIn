...
const EditDocument = () => {
  ...
  const REACT_APP_GOOGLE_DRIVE_API_KEY =
    "AIzaSyBnRYgL_mPNKHmEKteVJw-fcf7P3dCJIwI";
  const [PostFileDocument, { isLoading: loadingPostFileDocument }] =
    usePostFileDocumentMutation();
  const [DeleteFileDocument, { isLoading: loadingDeleteFileDocument }] =
    useDeleteFileDocumentMutation();
  const [PutDocument, { isLoading: isPutDocumentMutation }] =
    usePutDocumentMutation();
  const [DeleteDocument, { isLoading: isDeleteDocumentMutation }] =
    useDeleteDocumentMutation();
  ...
  const { data: DocumentsByDocumentId, isLoading: isDocumentsByDocumentId } =
  ...
  useEffect(() => {
    ...
    if (!isDocumentsByDocumentId && DocumentsByDocumentId) {
      ...
      setDeleteClass(DocumentsByDocumentId.anLopHoc === 1);
    }
  }, [isDocumentsByDocumentId, DocumentsByDocumentId]);
  const { data: FileDocumentsByUnitsId, isLoading: isFileDocumentsByUnitsId } =
  ...
  const removeFile = (indexToRemove, index) => {
    if (index === 1) {
      setDeletedFiles((currentList) => {
        return [...currentList, filesOld[indexToRemove]];
      });
      setFilesOld((prevFiles) =>
                ...
                <List>
                    ...
                    <List>
                      {filesOld.map((file, index) => (
                        ...
                        >
                          ...
                          <IconButton
                            onClick={(e) =>
                              handleFileClick(e, file.alternateLink)
                            }
                          >
                            <FileOpenIcon />
                          </IconButton>
                          {userId === teacherId && !deleteClass && (
                            <IconButton onClick={() => removeFile(index, 1)}>
                              <CloseIcon />
                            </IconButton>
                          )}
                        </ListItem>
                  ...
                  {files.map((file, index) => (
                    ...
                    >
                      {getFileIcon(file.name)}
                      <ListItemText primary={file.name} />
                      <IconButton onClick={() => removeFile(index, 2)}>
                        <CloseIcon />
                      </IconButton>
                    </ListItem>
                  ))}
