...
const ClassWidget = ({ classItem, userId }) => {
  ...
  const [unitsGrade, setUnitsGrade] = React.useState([]);
  const [testsGrade, setTestsGrade] = React.useState([]);
  const [currentUnitGrade, setCurrentUnitGrade] = useState(null);
  useEffect(() => {
    if (unitsData) {
      setUnitsGrade(
        unitsData.map((unit) => ({
          id: unit.chuong.ma_chuong,
          label: unit.chuong.ten,
        })),
      );
      setCurrentUnitGrade({
        id: unitsData[0].chuong.ma_chuong,
        label: unitsData[0].chuong.ten,
      });
      setTestsGrade(
        unitsData
          .filter((unit) => unit.chuong.ma_chuong === currentUnitGrade?.id)
          .map((unit) => unit.deKiemTra)
          .flat()
          .map((test) => ({
        ...
        <TabContext value={value}>
          ...
          >
            ...
            >
              ...
              >
                <MenuItem onClick={(handleCloseCreateMenu, handleOpenEditUnit)}>
                  <FlexBetween>
              ...
              <Box>
                ...
                >
                  ...
                  <Autocomplete
                    ...
                    size="small"
                    onChange={(event, value) => setCurrentUnitGrade(value)}
                    value={currentUnitGrade.label}
