...
@router.post("/{ma_cauHoi}",status_code=status.HTTP_201_CREATED,response_model=schemas.CauTraLoi)
async def create(schema_object: schemas.CauTraLoiCreate,ma_cauHoi: str, db: Session = Depends(database.get_db)):
...
@router.get("/",response_model=list[schemas.CauTraLoi],status_code=status.HTTP_200_OK)
...
async def read(ma_cauTraLoi: str, db: Session = Depends(database.get_db)):
    db_object = db.query(models.CauTraLoi).filter(models.CauTraLoi.ma_cauTraLoi == ma_cauTraLoi).first()
    if db_object is None:
        raise HTTPException(status_code=400, detail="CauHoi not found")
    return db_object


@router.get("/cauHoi/{ma_cauHoi}",response_model=list[schemas.CauTraLoi],status_code=status.HTTP_200_OK)
async def read(ma_cauHoi: str, db: Session = Depends(database.get_db)):
    ...
    db_object = db.query(models.CauTraLoi).filter(models.CauTraLoi.ma_cauHoi == ma_cauHoi).all()
    return db_object
