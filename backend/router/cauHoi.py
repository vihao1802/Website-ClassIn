...
@router.post("/{ma_taiKhoan}",status_code=status.HTTP_201_CREATED,response_model=schemas.CauHoi)
async def create(schema_object: schemas.CauHoiCreate,ma_taiKhoan: str, db: Session = Depends(database.get_db)):
...
@router.get("/",response_model=list[schemas.CauHoi],status_code=status.HTTP_200_OK)
...
async def read(ma_cauHoi: str, db: Session = Depends(database.get_db)):
    db_object = db.query(models.CauHoi).filter(models.CauHoi.ma_cauHoi == ma_cauHoi).first()
    if db_object is None:
        raise HTTPException(status_code=400, detail="CauHoi not found")
    return db_object


@router.get("/taiKhoan/{ma_taiKhoan}",response_model=list[schemas.CauHoi],status_code=status.HTTP_200_OK)
async def read(ma_taiKhoan: str, db: Session = Depends(database.get_db)):
    ...
    db_object = db.query(models.CauHoi).filter(models.CauHoi.ma_taiKhoan == ma_taiKhoan).all()
    return db_object
