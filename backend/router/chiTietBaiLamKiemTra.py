...
router = APIRouter(
    prefix="/chiTietBaiLamKiemTra",
    tags=['chiTietBaiLamKiemTra']
)


@router.post("/",status_code=status.HTTP_201_CREATED,response_model=schemas.ChiTietBaiLamKiemTra)
async def create(schema_object: schemas.ChiTietBaiLamKiemTraCreate,db: Session = Depends(database.get_db)):
    ...
    db_object_check = db.query(models.BaiLamKiemTra).filter(models.BaiLamKiemTra.ma_baiLamKiemTra == ma_baiLamKiemTra).first()
    if db_object_check is None:
        raise HTTPException(status_code=400, detail="ma_baiLamKiemTra not found")
...
@router.get("/",response_model=list[schemas.ChiTietBaiLamKiemTra]  ,status_code=status.HTTP_200_OK)
...
async def read(ma_baiLamKiemTra: str, db: Session = Depends(database.get_db)):
    db_object = db.query(models.ChiTietBaiLamKiemTra).filter(models.ChiTietBaiLamKiemTra.ma_baiLamKiemTra == ma_baiLamKiemTra).all()
    if not db_object:
