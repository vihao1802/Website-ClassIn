...
@router.post("/{ma_lopHoc}",status_code=status.HTTP_201_CREATED,response_model=schemas.Chuong)
async def create(schema_object: schemas.ChuongCreate,ma_lopHoc: str,db: Session = Depends(database.get_db)):
...
@router.get("/",response_model=list[schemas.Chuong]  ,status_code=status.HTTP_200_OK)
...
@router.get("/lopHoc/{ma_lopHoc}",response_model=list[schemas.Chuong],status_code=status.HTTP_200_OK)
async def read(ma_lopHoc: str, db: Session = Depends(database.get_db)):
    db_object = db.query(models.Chuong).filter(models.Chuong.ma_lopHoc == ma_lopHoc).all()
    if not db_object:
        raise HTTPException(status_code=400, detail="ma_lopHoc not found")
    return db_object