...
router = APIRouter(prefix="/luuVetBaiLamKiemTra", tags=["luuVetBaiLamKiemTra"])


@router.post(
    "/",
    status_code=status.HTTP_201_CREATED,
    response_model=schemas.LuuVetBaiLamKiemTra,
)
async def create(
    schema_object: schemas.LuuVetBaiLamKiemTraCreate,
...
@router.get(
    "/",
    response_model=list[schemas.LuuVetBaiLamKiemTra],
...
async def read(
    ma_luuVetBaiLamKiemTra: str, db: Session = Depends(database.get_db)
):
    db_object = (
        db.query(models.LuuVetBaiLamKiemTra)
        .filter(
            models.LuuVetBaiLamKiemTra.ma_luuVetBaiLamKiemTra
            == ma_luuVetBaiLamKiemTra
        )
        .first()
    )
    if db_object is None:
        raise HTTPException(
            status_code=400, detail="LuuVetBaiLamKiemTra not found"
        )
    return db_object
