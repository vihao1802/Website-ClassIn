...
@router.post("/{ma_chuong}",status_code=status.HTTP_201_CREATED,response_model=schemas.HocLieu)
async def create(schema_object: schemas.HocLieuCreate,ma_chuong: str,db: Session = Depends(database.get_db)):
...
@router.get("/",response_model=list[schemas.HocLieu]  ,status_code=status.HTTP_200_OK)
