...
@router.post("/{ma_taiKhoan}",status_code=status.HTTP_201_CREATED,response_model=schemas.BaiLamKiemTra)
async def create(schema_object: schemas.BaiLamKiemTraCreate,ma_taiKhoan: str, db: Session = Depends(database.get_db)):
    ...
    db_object = models.BaiLamKiemTra(**schema_object.dict(), ma_taiKhoan=ma_taiKhoan)
    db.add(db_object)
    db.commit()
...
@router.get("/cauHoi/{ma_taiKhoan}",response_model=list[schemas.BaiLamKiemTra],status_code=status.HTTP_200_OK)
